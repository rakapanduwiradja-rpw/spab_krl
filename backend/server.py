...
class PencatatanCreate(BaseModel):
    qr_code: str
    angka_meter_akhir: float
    foto_meter: Optional[str] = None
    koordinat_gps: Optional[str] = None
    catatan: Optional[str] = ""
    periode: Optional[str] = None  # ISO date string; default = bulan ini
...
async def create_pencatatan(body: PencatatanCreate, user: dict = Depends(require_any)):
    ...
    if not p:
        raise HTTPException(404, "QR Code tidak dikenali")
    # Gunakan periode dari request jika ada, fallback ke bulan ini
    if body.periode:
        try:
            parsed = datetime.fromisoformat(body.periode.replace("Z", "+00:00"))
            periode = iso(datetime(parsed.year, parsed.month, 1, tzinfo=timezone.utc))
        except Exception:
    ...
    last = await db.pencatatan.find_one({"id_pelanggan": p["id"]}, {"_id": 0}, sort=[("periode_bulan", -1)])
    angka_lalu = last["angka_meter_akhir"] if last else p.get("angka_awal", 0)
    if body.angka_meter_akhir < angka_lalu:
        raise HTTPException(400, f"Angka meter akhir ({body.angka_meter_akhir}) < meter awal ({angka_lalu})")

    pemakaian = round(float(body.angka_meter_akhir) - float(angka_lalu), 2)
    ...
    catat_doc = {
        ...
        "angka_meter_awal": angka_lalu,
        "angka_meter_akhir": body.angka_meter_akhir,
    ...
    await db.pelanggan.update_one(
        {"id": p["id"]}, {"$set": {"angka_meter_terakhir": body.angka_meter_akhir}}
    )
...
async def get_tagihan(tid: str, user: dict = Depends(require_any)):
    ...
    s = await db.pengaturan.find_one({"id": "main"}, {"_id": 0}) or {}
    # Enrich tagihan dengan angka meter dari pencatatan
    if t.get("id_pencatatan"):
        catat = await db.pencatatan.find_one({"id": t["id_pencatatan"]}, {"_id": 0})
        if catat:
            t.setdefault("angka_meter_awal", catat.get("angka_meter_awal", 0))
            t.setdefault("angka_meter_akhir", catat.get("angka_meter_akhir", 0))
    return ok({"tagihan": t, "pelanggan": p, "pengaturan": s})


# ── Endpoint alias untuk frontend (dipakai di Nota.jsx dan NotaPetugas.jsx) ──

@api.get("/tagihan_detail")
async def tagihan_detail_alias(id: str, user: dict = Depends(require_any)):
    """Alias GET /tagihan_detail?id=<tid> — dipakai oleh halaman cetak nota."""
    t = await db.tagihan.find_one({"id": id}, {"_id": 0})
    if not t:
        raise HTTPException(404, "Tagihan tidak ditemukan")
    p = await db.pelanggan.find_one({"id": t["id_pelanggan"]}, {"_id": 0})
    s = await db.pengaturan.find_one({"id": "main"}, {"_id": 0}) or {}
    # Enrich tagihan dengan angka meter dari pencatatan agar nota lengkap
    if t.get("id_pencatatan"):
        catat = await db.pencatatan.find_one({"id": t["id_pencatatan"]}, {"_id": 0})
        if catat:
            t.setdefault("angka_meter_awal", catat.get("angka_meter_awal", 0))
...
@api.get("/tagihan_list")
async def tagihan_list_alias(
    ...
    """Alias GET /tagihan_list — dipakai oleh daftar nota petugas."""
    flt = {}
    if status:
        flt["status_bayar"] = status
    if rt:
...
async def dashboard_stats(pencatatan_periode: Optional[str] = None, user: dict = Depends(require_any)):
    # KPI cards always use current month
    periode_kpi = iso(month_start(now_utc()))
    # Pencatatan progress uses configurable periode, defaults to previous month
    if pencatatan_periode:
        try:
            parsed = datetime.fromisoformat(pencatatan_periode.replace("Z", "+00:00"))
            periode_catat = iso(datetime(parsed.year, parsed.month, 1, tzinfo=timezone.utc))
        except Exception:
            periode_catat = periode_kpi
    else:
    ...
    total_catat = await db.pencatatan.count_documents({"periode_bulan": periode_catat})
    tagihan_bulan = await db.tagihan.find({"periode_bulan": periode_kpi}, {"_id": 0}).to_list(5000)
    ...
    # per RT progress (based on pencatatan periode)
    rt_stats = []
    for rt in ["RT01", "RT02", "RT03"]:
        ...
        sudah = await db.pencatatan.count_documents({
            "periode_bulan": periode_catat, "id_pelanggan": {"$in": ids}
        })
        anomali = await db.pencatatan.count_documents({
            "periode_bulan": periode_catat, "id_pelanggan": {"$in": ids}, "is_anomali": True
        })
        # List of pelanggan belum dicatat
        sudah_ids = [p["id_pelanggan"] async for p in db.pencatatan.find(
            {"periode_bulan": periode_catat, "id_pelanggan": {"$in": ids}}, {"id_pelanggan": 1, "_id": 0}
        )]
        belum_list = []
        for bid in ids:
            if bid not in sudah_ids:
                p = await db.pelanggan.find_one({"id": bid}, {"nama": 1, "rt": 1, "_id": 0})
                if p:
                    belum_list.append({"id": bid, "nama": p["nama"], "rt": p["rt"]})
        rt_stats.append({
    ...
    return ok({
        ...
        "rt_stats": rt_stats,
        "progres_rt": [{"rt": r["rt"], "total": r["total"], "dicatat": r["dicatat"], "belum": r["belum"], "anomali": r["anomali"], "belum_list": r["belum_list"]} for r in rt_stats],
...
async def tren_pemakaian(rt: Optional[str] = None, bulan: int = 12,
                         semua: bool = False, periode_spesifik: Optional[str] = None,
                         user: dict = Depends(require_admin)):
    start = month_start(now_utc())
    labels = []
    if periode_spesifik:
        # filter 1 bulan spesifik
        try:
            parsed = datetime.fromisoformat(periode_spesifik.replace("Z", "+00:00"))
            labels = [iso(datetime(parsed.year, parsed.month, 1, tzinfo=timezone.utc))]
        except Exception:
            labels = [iso(start)]
    elif semua:
        # ambil semua periode yang ada di database
        all_periods = await db.pencatatan.distinct("periode_bulan")
        all_periods += await db.tagihan.distinct("periode_bulan")
        labels = sorted(set(all_periods))
    else:
