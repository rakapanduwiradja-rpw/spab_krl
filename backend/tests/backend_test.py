...
class TestPelanggan:
    ...
    def test_scan_known_qr(self, admin_token):
        ...
        assert r.status_code == 200
        d = r.json()["data"]
        assert "pelanggan" in d and "angka_meter_awal" in d
...
class TestPencatatan:
    def test_create_pencatatan_and_tagihan(self, petugas_token, admin_token):
        ...
        for p in lst:
            ...
            angka_lalu = scan["angka_meter_awal"]
            chosen = (p, angka_lalu)
            break
        assert chosen
        p, angka_lalu = chosen
        ...
        r = requests.post(f"{API}/pencatatan", headers=H(petugas_token), json={
            "qr_code": p["qr_code"],
            "angka_meter_akhir": angka_lalu + 5.5,
        })
        ...
        if r.status_code == 200:
            ...
            r2 = requests.post(f"{API}/pencatatan", headers=H(petugas_token), json={
                "qr_code": p["qr_code"], "angka_meter_akhir": angka_lalu + 6,
            })
    ...
    def test_pencatatan_invalid_meter(self, petugas_token, admin_token):
        ...
        p = lst[1]
        r = requests.post(f"{API}/pencatatan", headers=H(petugas_token), json={
            "qr_code": p["qr_code"], "angka_meter_akhir": 0.0,
        })
        assert r.status_code == 400
