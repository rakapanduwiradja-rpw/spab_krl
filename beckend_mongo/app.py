...
class _DocSnapshot:
    ...
    @property
    def reference(self):
        return _DocRef(self._collref, self.id)


class _DocRef:
    def __init__(self, collref, doc_id):
        self._collref = collref
        self.id = doc_id

    def get(self):
        raw = self._collref._coll.find_one({"_id": self.id})
        snap = _DocSnapshot(self.id, raw)
        snap._collref = self._collref
        return snap

    def set(self, data, merge=False):
        if merge:
            self._collref._coll.update_one({"_id": self.id}, {"$set": dict(data)}, upsert=True)
        else:
            doc = dict(data)
            doc["_id"] = self.id
            self._collref._coll.replace_one({"_id": self.id}, doc, upsert=True)

    def update(self, data):
        self._collref._coll.update_one({"_id": self.id}, {"$set": dict(data)})

    def delete(self):
        self._collref._coll.delete_one({"_id": self.id})


class _Query:
    def __init__(self, collref):
        self._collref = collref
        self._filters = []
    ...
    def _mongo_filter(self):
        mf = {}
        for field, op, value in self._filters:
            if op == "==":
                mf[field] = value
            elif op == "in":
                mf[field] = {"$in": list(value)}
            elif op == ">=":
                mf.setdefault(field, {})["$gte"] = value
            elif op == "<=":
                mf.setdefault(field, {})["$lte"] = value
            elif op == ">":
                mf.setdefault(field, {})["$gt"] = value
            elif op == "<":
                mf.setdefault(field, {})["$lt"] = value
            else:
                raise ValueError(f"Operator query belum didukung: {op}")
        return mf

    def stream(self):
        cursor = self._collref._coll.find(self._mongo_filter())
...
class _Db:
    def collection(self, name):
        return _CollectionRef(_mongo_db[name])


# Shim supaya baris kode lama "firestore.Query.DESCENDING" tetap jalan tanpa diubah
firestore = SimpleNamespace(Query=SimpleNamespace(DESCENDING=-1, ASCENDING=1))
...
def petugas_reset_password(req):
...
ROUTES = {
    ...
    "seed_init": seed_init,
    "petugas_update": petugas_update,
    "petugas_reset_password": petugas_reset_password,
    "petugas_nonaktif": petugas_nonaktif,
    "pencatatan_update": pencatatan_update,
    "pencatatan_hapus": pencatatan_hapus,
    "pelanggan_hapus": pelanggan_hapus,
    "meteran_scan": meteran_scan,
}

@app.route("/", methods=["GET"])
def health():
    return jsonify({"success": True, "message": "SPAB KRL backend is running"})

@app.route("/<endpoint>", methods=["GET", "POST", "PUT", "DELETE", "OPTIONS"])
def dispatch(endpoint):
    handler = ROUTES.get(endpoint)
    if handler is None:
        return jsonify({"success": False, "message": f"Endpoint '{endpoint}' tidak ditemukan"}), 404
